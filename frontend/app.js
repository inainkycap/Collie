...
function setActiveStep(step) {
  ...
  steps.forEach((id, i) => {
    ...
    const active = i <= step;
    el.className = active
      ? "px-3 py-1 rounded-full text-xs font-medium bg-indigo-600 text-white"
...
function selectPill(container, btn) {
  ...
  btn.className =
    "pill px-3 py-2 rounded-full border border-indigo-200 bg-indigo-600 text-white shadow-sm transition text-sm";
}

export async function initIndex() {
  $("#createBtn").addEventListener("click", async () => {
    const title = $("#tripTitle").value.trim() || "Weekend Trip";
    const data = await apiFetch("/trip", { method: "POST", body: JSON.stringify({ title }) });
    ls.set("trip_id", data.trip_id);
...
export async function initTrip() {
  ...
  await refresh(tripId);
  setInterval(() => refresh(tripId), 2000);
